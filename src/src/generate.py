...
class GazeboObjectManager(Node):
    def __init__(self):
        ...
        # 等待服务可用
        self.get_logger().info('等待/spawn_entity服务可用（超时时间20秒）...')
        if not self.spawn_client.wait_for_service(timeout_sec=20):
        ...
        self.get_logger().info('等待/delete_entity服务可用（超时时间20秒）...')
        if not self.delete_client.wait_for_service(timeout_sec=20):
    ...
    def spawn_cylinder(self, name, x, y, z=0.0, radius=0.1, height=0.5):
        """同步生成圆柱体，返回服务响应"""
        self.get_logger().info(f'准备生成圆柱体: {name} 在位置 ({x}, {y}, {z})')
    ...
    def generate_random_obstacles(self, num_obstacles, x_range=(-5, 5), y_range=(-5, 5)):
        """生成随机障碍物（同步版本）"""
        self.get_logger().info(f"开始生成 {num_obstacles} 个障碍物")
        obstacles = []
        ...
        for i in range(num_obstacles):
            ...
            name = f"cylinder_{i}"
            self.get_logger().info(f"生成新障碍物: {name} 在 ({x:.2f}, {y:.2f})")
            if self.spawn_cylinder(name, x, y):
...
def training_loop():
    ...
    try:
        for episode in range(10):
            ...
            obstacles = manager.generate_random_obstacles(5)
